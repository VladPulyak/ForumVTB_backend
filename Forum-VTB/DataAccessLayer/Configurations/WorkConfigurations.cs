...
namespace DataAccessLayer.Configurations
{
    public class WorkConfigurations : IEntityTypeConfiguration<Work>
    {
        public void Configure(EntityTypeBuilder<Work> builder)
        {
            ...
            builder.HasOne(q => q.Subsection).WithMany(w => w.Works).HasForeignKey(q => q.SubsectionId).OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(q => q.User).WithMany(w => w.Works).HasForeignKey(q => q.UserId).OnDelete(DeleteBehavior.Cascade);
