...
namespace DataAccessLayer.Configurations
{
    public class JobConfigurations : IEntityTypeConfiguration<Job>
    {
        public void Configure(EntityTypeBuilder<Job> builder)
        {
            ...
            builder.HasOne(q => q.Subsection).WithMany(w => w.Jobs).HasForeignKey(q => q.SubsectionId).OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(q => q.User).WithMany(w => w.Jobs).HasForeignKey(q => q.UserId).OnDelete(DeleteBehavior.Cascade);
